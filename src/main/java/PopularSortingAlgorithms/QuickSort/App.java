package PopularSortingAlgorithms.QuickSort;
...
public class App {
    ...
    public static void quickSort(int[] inputArray, int start, int end) {
        ...
        if(start < end) {
            ...
            quickSort(inputArray,start,pp-1); // sorts left side of the range
            quickSort(inputArray,pp+1, end); //sort the right side of the range
        }
    ...
    public static int partition(int[] inputArray, int start, int end) {
        ...
        //in the first iteration i starts at -1
        int i = start -1;

        for(int j = start; j <= end-1; j++) {
            if(inputArray[j] <= pivot) {
                i++;
                //we are swaping below
